...
export default function SynastryVTQuaterM({ synastry, date }: { synastry: Synastry, date: SplittedDate }) {
  const listOfMonths = synastry.getCustomMonths();
  const allMonths = getAllMonths();
  const quater1 = synastry.getQuarterOne();
  const quater2 = synastry.getQuarterTwo(date.year);
  const quater3 = synastry.getQuarterThree(date.year);
  const lastYear = date.year - 1;
  const quater1LastYear = synastry.getQuarterOne();
  const quater2LastYear = synastry.getQuarterTwo(lastYear);
  const quater3LastYear = synastry.getQuarterThree(lastYear);
  const quater1Karmico = synastry.getQuarterOneISK();
  const quater2Karmico = synastry.getQuarterTwoISK(date.year);
  const quater3Karmico = synastry.getQuarterThreeISK(date.year);
  const quater2KarmicoLast = synastry.getQuarterTwoISK(lastYear);
  const quater3KarmicoLast = synastry.getQuarterThreeISK(lastYear);
  const personalYearISK = synastry.calcPersonalYearISK(date.year);
